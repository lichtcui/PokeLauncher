...
- 作弊（离线，需先开启「作弊模式」）：首页就绪页「作弊设置」选择条目，进入游戏后**实时生效**（无需重启）；条目可扩展（见 [`AGENTS.md`](./AGENTS.md) §10）
...
所有条目**实时生效**：进入游戏（开始或继续一局）后，开关与数值立即作用于当前对局，**无需重启**。
...
已在真机（Pura 70 Pro+ / API 24）验证：下载（镜像 ~1.4MB/s）、解压、离线游戏、启动加速、存档导出、后台静音、全屏与安全区适配、首页极简改版、设置页（检查更新/刷新缓存/删除）、屏幕方向（竖屏/横屏/跟随系统）、作弊（场景捕获 + 实时 hook：糖果/经验/金币倍率、队伍红闪、扭蛋券数量）。
