# PokeLauncher

在 HarmonyOS NEXT 手机上**离线游玩 PokeRogue** 的启动器（原生 HAP）。

> 本 App 只是「壳 + 启动器」，**不含游戏本体**：首次启动时由 App 从公开来源下载游戏资源并解压到本机，之后即可断网游玩。

## 安装

> 当前 [GitHub Release](https://github.com/lichtcui/PokeLauncher/releases) **仅提供源码，未附带 HAP**：本地构建产物使用 DevEco 自动签名（debug Profile，绑定开发者设备 UDID），其他人下载后无法安装。详见 [`docs/app-update.md`](./docs/app-update.md)。

1. 克隆仓库并自行构建（需先用 DevEco Studio 打开一次，在 `File > Project Structure > Signing Configs` 配置自己的签名）：

   ```bash
   git clone https://github.com/lichtcui/PokeLauncher.git
   hvigorw assembleHap --mode module -p product=default -p buildMode=release
   ```

2. 把 HAP 传到手机安装：`hdc install -r entry/build/default/outputs/default/entry-default-signed.hap`
3. 系统要求：HarmonyOS NEXT（API 24 及以上）。

> 侧载应用无法在 App 内自动安装新版本，更新 App 需手动重新构建并安装 HAP（App 内会提示有新版本）。

## 首次使用

1. 打开 App，首页会显示「开始下载」，点它开始下载。
2. 等待下载 + 解压完成（国内镜像自动测速选源，支持暂停 / 继续 / 取消）。
3. 出现资源图标和「开始运行」后，点进去即可。

> 下载太慢时：首页会显示本次资源包的 **GitHub 直链**（点击复制），用浏览器或下载工具取得同名 `game.zip` 传到手机，回到首页点「上传资源」选中它即可（跳过下载与测速，直接解压安装）。直链随游戏版本自动更新，无需手动维护。

游戏为**离线运行**，下载一次后不再需要联网（除非更新游戏资源）。

## 资源来源与更新原理

- **来源**：游戏资源**不随 App 打包**。App 通过 GitHub API 查询 PokeRogue 官方公开仓库的 latest release（`api.github.com/repos/Admiral-Billy/pokerogue/releases/latest`），下载其中的 `game.zip` 资产。
- **镜像加速**：直连 GitHub 慢时，按顺序尝试第三方代理 `gh-proxy.com` → `ghfast.top` → `ghproxy.net`（先做小范围测速选最快的），全部失败再回退 GitHub 直连；API 查询同样支持镜像。
- **下载与安装**：支持断点续传（临时 `.part` 文件），下载完成后解压到沙箱 `files/game/`，原子切换后即就绪；「离线资源更新」即重新拉取 latest。
- **兜底导入**：网络不可用或下载过慢时，可复制首页显示的**资源包直链**（当次 `game.zip` 的 GitHub 地址）自行获取同名 `game.zip`，再点「上传资源」选中它，走同一套解压 / 切换流程。
- **App 本体更新**：与游戏资源相互独立，见 [更新](#更新) 一节。

> 实现细节见 [`docs/architecture.md`](./docs/architecture.md)。

## 功能说明

### 首页启动器

- 四种状态：未下载 / 下载中 / 解压中 / 已就绪。
- 已就绪后：中间资源图标 + 「开始运行」+「参数调整」（需先在设置里开启），右上角齿轮进设置。
- 下载中可暂停、继续、取消（取消需二次确认）。

### 游戏页

- 右上角「返回」按钮 3 秒后自动淡出，避免遮挡游戏信息；点一下可唤回，再点才返回。
- 系统返回手势同样可以退出游戏页。
- 切到后台自动静音 BGM，回到前台恢复。
- 屏幕方向可在设置里选：竖屏 / 横屏 / 跟随系统（只对游戏页生效，首页和设置页恒为竖屏）。

### 设置页

- **屏幕方向**：竖屏 / 横屏 / 跟随系统。
- **参数调整**：总开关，开启时需确认账号风险与兼容性风险。
- **数据管理**：
  - 应用更新：手动检查 App 新版本（发现新版本弹窗引导到下载页）。
  - 离线资源更新：重新下载最新的离线资源包。
  - 删除本地数据：清掉已下载的离线资源。
- **作者信息**：作者与 GitHub 仓库地址。
- **关于 / 法律与免责声明**。

### 存档导入 / 导出

- **存档位置**：App 沙箱的 Web `localStorage`（物理路径 `<沙箱>/cache/web/`），并绑定本地服务地址 `http://127.0.0.1:18787`（端口固定，不会变）。
- **导出**：游戏内「导出存档」会通过文件选择器保存到 `下载/com.lichtcui.pokelauncher/`。
- **导入**：通过文件选择器选择存档文件导入。
- **快照兜底**：App 会把 localStorage 镜像一份到 `<沙箱>/files/saves/`。若「清除缓存」把 localStorage 清空，进入游戏页时会自动从快照恢复，并弹提示告知。

**什么时候会丢存档**

| 操作 | 存档 |
| --- | --- |
| 设置页「删除本地数据」（只删离线资源） | 保留 |
| 离线资源更新 | 保留 |
| 覆盖安装 App（同签名） | 保留 |
| 系统设置或 `bm clean -c` 清除缓存 | 由快照自动恢复 |
| 卸载重装 / `bm clean -d` 清数据 | **丢失** |
| 换签名导致必须先卸载 | **丢失** |

> 最可靠的做法：定期用游戏内「导出存档」存一份到 `下载/`——它在沙箱外，卸载重装也不受影响。

## 参数调整（离线）

在「设置 → 参数调整」开启后，首页就绪页会出现「参数调整」入口。条目分两组：

- **倍率修改**：糖果 / 经验 / 金币获取倍率（×N）
- **其他**：幸运值拉满（SSS）、100% 捕获率、免费抽蛋、强制奖励稀有度

所有条目均为**实时 hook**，注入后在游戏内立即生效；但**更改开关或数值需要重新进入游戏页**（返回首页再进，或重启 App）才会重新注入。

### 风险

- **账号风险**：PokéRogue 官方有检测机制，修改数据可能被标记（Flagged）甚至封禁；修改后的存档不纯净，**勿导入在线版**。
- **兼容性风险**：实时调整依赖游戏内部方法名。本 App 会从 GitHub Release 自动更新游戏资源，**游戏升级后可能失效或表现异常**（届时关闭即可）。

## 更新

- **离线资源**：设置页「离线资源更新」可手动更新到最新资源包（也可随时「删除本地数据」后重新下载）。
- **App 本体**：启动时自动检查（24 小时节流）+ 设置页「应用更新」手动检查；发现新版本会弹窗（强制更新时不可关闭），点「去更新」用浏览器打开下载页，自行构建并安装（见「安装」一节）。

## 常见问题

| 现象 | 处理 |
| --- | --- |
| 下载失败 / 很慢 | 换个网络重试；App 会自动在多个镜像间测速选源；实在不行自行获取 `game.zip` 后用首页「上传资源」导入 |
| 进入游戏白屏 | 返回首页重新进；仍不行就重启 App |
| 参数调整没生效 | 改完配置要**重新进入游戏页**；确认总开关和对应条目都已打开 |
| 游戏更新后参数调整异常 | 属正常现象，关闭即可（依赖游戏内部实现） |
| 存档丢了 | 见「存档导入 / 导出」的丢失场景表；用导出的存档文件重新导入即可恢复 |

更多问题见 [`docs/troubleshooting.md`](./docs/troubleshooting.md)。

## 许可与免责

- 本项目（启动器代码）以 [MIT License](./LICENSE) 发布。
- PokeRogue 及其游戏资源的版权归原作者及贡献者所有，**本项目不再分发任何游戏本体**（仅运行时由用户设备从公开来源下载）。
- 本项目为第三方非官方启动器，与 PokeRogue 官方无隶属、合作或授权关系，仅供个人学习交流与离线游玩使用。
- 完整条款见 App 内「设置 → 关于 → 法律与免责声明」。

---

开发 / 编译 / 架构说明见 [`AGENTS.md`](./AGENTS.md) 与 [`docs/`](./docs)。
